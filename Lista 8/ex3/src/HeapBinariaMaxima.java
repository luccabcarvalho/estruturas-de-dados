public class HeapBinariaMaxima {
	...
	public HeapBinariaMaxima(HeapBinariaMinima heapMinima)
	{
		...
		for( int i = 0; i < tam; i++ )
			vetor[ i + 1 ] = heapMinima.getElemento(i + 1);
	...
	/* Busca o maior item na fila de prioridades.
	   Retorna o maior item em itemMin e trn  ue, ou falso se a heap estiver vazia. */
	public int max()
	{
		if (this.vazia())
		{
	...
	public boolean verificaPropriedadeHeap(int[] vetor)
	{
		...
		for(int i = 1; i < n/2 ; i++)
		{
			if(vetor[i] < vetor[2 * i] && 2 * i < n)
				return false;

			else if(vetor[i] < vetor[2 * i + 1] && 2 * i < n)
				return false;
		}
