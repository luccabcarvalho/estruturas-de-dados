...
public class ArvBinBusca<Chave extends Comparable<Chave>, Valor> 
{
    ...
    */
    ...
    {
        if(chave == null)
            System.out.printf("Arvore vazia");
    ...
    public void constroiArvore(int[] vetor)
    {
        ...
        {
            insere((Chave)((Integer) vetor[i]), (Valor)((Integer) vetor[i]));
        }
    }
    ...
    //Complexidade O(n)
    public boolean eSimilar(ArvBinBusca arvore)
    {
        boolean result = eSimilar(raiz, arvore.raiz);
        return result;
    }

    public boolean eSimilar(No x, No y)
    {
        if((x == null) && (y == null))
            return true;
            ...
            if((x.chave == y.chave) && eSimilar(x.esq, y.esq) == true && eSimilar(x.dir, y.dir) == true)
                return true;
            else
                return false;
        }
    ...
    {
        int tamanhoInicio = tamanho();
        int tamanhoFinal;

        removeForaIntervalo(chaveMin, chaveMax, raiz);

        tamanhoFinal = tamanho();

        if(tamanhoInicio != tamanhoFinal)
            return true;
        else
            return false;
    }

    private void removeForaIntervalo(Chave chaveMin, Chave chaveMax, No x)
    {
        int cmpMin = x.chave.compareTo(chaveMin);
        int cmpMax = x.chave.compareTo(chaveMax);

        if(x.esq != null)
            removeForaIntervalo(chaveMin, chaveMax, x.esq);
        if(x.dir != null)
            removeForaIntervalo(chaveMin, chaveMax, x.dir);
    ...
    //Complexidade O(h)
    //Retorno e parâmetros dos métodos como "Chave" em razão da comparação entre os nós
    public Chave obtemAncestralComum(Chave primeiroNo, Chave segundoNo)
    {
        Chave result = obtemAncestralComum(raiz, primeiroNo, segundoNo);
        return result;
    }

    public Chave obtemAncestralComum(No x, Chave primeiroNo, Chave segundoNo)
    {
        int cmp1 = x.chave.compareTo(primeiroNo);
        int cmp2 = x.chave.compareTo(segundoNo);
    ...
    private int ebalanceada(No x)
    {
        ...
        alturaEsquerda = ebalanceada(x.esq);

        if(alturaEsquerda == -1)
            return -1;

        alturaDireita = ebalanceada(x.dir);
